...
namespace scpp
{
    ...
    SocketCanStatus SocketCan::write(const CanFrame &msg)
    {
        ...
        // std::cout << "Length of can frame:" << msg.len << "\n";
        if (m_socket_mode == MODE_CANFD_MTU)
        {
            /* ensure discrete CAN FD length values 0..8, 12, 16, 20, 24, 32, 64 */
            frame.len = can_dlc2len(can_len2dlc(frame.len));
        }
        // std::cout << "Length of can frame:" << frame.len << "\n";
        /* send frame */
        if (::write(m_socket, &frame, int(m_socket_mode)) != int(m_socket_mode))
        {
            std::cout << "Return of CAN-write:" << ::write(m_socket, &frame, int(m_socket_mode)) << "\n";
            perror("write");
    ...
    void SocketCan::send(int data[], const int n, const int id)
    {
        scpp::CanFrame cf_to_write;
        cf_to_write.id = id;
        cf_to_write.len = n;
